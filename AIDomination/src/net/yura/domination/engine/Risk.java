...
*/
				...
				if (StringT.hasMoreTokens()==false) {
					...
					if ( chatSocket == null ) { // LOCAL GAME/ OR LOBBY CLIENT
						...
						if ( myAddress.equals(Addr) ) { // only on the pc of who called it
							...
							if (game.getCurrentPlayer()!=null && !(game.getCurrentPlayer().getAddress().equals(Addr) && game.getCurrentPlayer().getAI().getId().equals("human"))) {
			 ...
			 */
				...
				if (input.equals("choosemap")) {
					...
					if (StringT.countTokens()>=3) {
						...
						if (color != 0	&& ai != null	&& !name.equals("")
								&& ((unlimitedLocalMode && game.addPlayer(ai,name, color, "LOCALGAME")) || (!unlimitedLocalMode && game.addPlayer(ai, name, color, Addr)))) {
	...
	public static String getType(int type) {
		...
		case Player.PLAYER_AI_VERY_HARD: return "ai veryhard";
	...
	private boolean showHumanPlayerThereInfo(Player p) {
		...
		return (p != null) && ( p.getAI().getId().equals("human") ) && ( unlimitedLocalMode || myAddress.equals( p.getAddress() ) );
